...
class MEDL(object):
    ...
    def __init__(self, model, params, e_step_iter = 10, min_iterations=10, gll_early_stop_threshold=1e-4, max_iterations=20):
    ...
    def fit(self, X, Z, clusters, y):
        ...
        while iteration < self.max_iterations:
            ...
            # Do the deep learning regression with all the fixed effects features

            for e_iterations in range(self.e_step_iter):

                self.model.fit(X, y_star, **self.params)

            f_hat = self.model.predict(X).reshape(len(y), )
